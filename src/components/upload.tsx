	...
	(
		{
			...
			size = 'small',
			multiple = false,
			formats = ['image/jpeg', 'image/png', 'image/jpg', 'image/gif', 'video/mp4'],
			onChange,
			...props
		},
		ref
		...
		return (
			<div
				className={cn(
					...
					isDragging ? 'border-rubricui-contrast' : 'border-rubricui-contrast/10',
					sizes[size],
					className
				)}
					...
					{files.length > 0 ? (
						<div className="mt-2 flex flex-col font-medium text-rubricui-contrast text-xs">
							...
							{files.map((file, index) => (
								<li className="line-clamp-1 overflow-ellipsis py-0.5" key={index}>
									{file.name}
								</li>
							))}
						</div>
						...
						<>
							...
							<p className="mt-1 text-rubricui-contrast/50 text-xs">
								{formats.map(format => format.split('/')[1]).join(', ')} formats
							</p>
						</>
					)}
				</div>
